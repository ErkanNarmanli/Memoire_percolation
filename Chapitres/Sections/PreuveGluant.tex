...
.					\sum_{\omega\in\mathcal{A}} \Pp{\omega}
	...
	\subsection{Preuve du lemme gluant}
		...
		\paragraph{Chemin minimal}
			Soit~$\omega$ une configuration, comme~$\leq$ est une relation totale sur l'ensemble fini des chemins auto-évitants entre~$\SnB$ et~$\ZnB$, on peut définir~$\Cmin$ \marginnote{$\Cmin$} comme étant l'élément minimal pour cette relation dans~$\omega$. Dès lors, on définit l'ensemble~$U(\omega)$ \marginnote{$U(\omega)$} comme étant l'ensemble des éléments de~$B_n'$ vérifiant les deux propriétés suivantes :
		...
		Pour la suite de la démonstration, on définit l'évènement suivant :
			...
			\begin{dem}[de la proposition\ref{prop:Ugrand}]
				...
				\begin{enumerate}
					\item\label{item:cons:a} On prend~$u,v,w$ \marginnote{$u,v,w$} trois points distincts qui sont  à distance~$1$ de~$z$  et telle que l'on puisse comparer les arrêtes avec~$(z,v)\prec (z,w)$.
					...
					Puisque~$z$ est dans~$U(\omega)$, par~\ref{item:P1}, il existe~$\pi'$ un chemin auto-évitant qui relie~$\overline{z+B_1}$ à~$S_n'$ ; on pose alors~$w'$ \marginnote{$w'$} le sommet de~$\partial\BRz$ par lequel~$\pi'$ sort pour la dernière fois de~$\BRz$ et~$\pi$ le chemin qui relie alors~$w'$ à~$S_n'$ et qui ne rencontre~$\BRz$ qu'en~$w'$. On a alors~$w'$ distinct de~$u'$ et~$v'$ car sinon on aurait~$S_n'$ qui serait relié à~$S_{3n}$, en passant par~$\Cmin$ jusqu'à~$w'$ puis ensuite par~$\pi$.
